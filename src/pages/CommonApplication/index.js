...
const CommonApplication = () => {
  ...
  const onLoadData = (date, page) => {
    if (date != null && date != null) {
      setLoading(true);
      getAllEntries(date.date_from, date.date_to, page, OFFSET).then((res) => {
        var dataSet = [];
          ...
          res.data.content.forEach((e) => {
            dataSet.push({
              ...
              option: (
                <div>
                  <button
                    onClick={() => downloadCsvFile(e)}
                    type="button"
                    className="btn btn-success btn-sm waves-effect waves-light"
                  >
                    <span className="d-flex">
                      <Spinner type="download" loading={downloading} /> Download
                    </span>
                  </button>
                  {getViewDocument(e)}
                </div>
  ...
  const getViewDocument = (e) => {
    if (e.hasDocument === true) {
      return (
        ...
        >
          <span className="d-flex">
            <Spinner type="images" loading={false} /> Documents
          </span>
        </Link>
      );
    } else {
  ...
  const downloadCsvFile = (e) => {
    setDownloading(true);
    getCsvFileByRefNo(e.referenceNo).then((status) => {
      if (status) {
  ...
  const downloadBulkCsvFile = () => {
    ...
    getCsvAllData(data.date_from, data.date_to).then((status) => {
      if (status) {
                    ...
                    <Row className="my-4">
                      ...
                      <Col className="col-4">
                        <div className="d-flex justify-content-end">
                          ...
                          >
                            <span className="d-flex">
                              <Spinner type="search" loading={loading} /> Search
                            </span>
                          </button>
                          <button
                            onClick={downloadBulkCsvFile}
                            type="button"
